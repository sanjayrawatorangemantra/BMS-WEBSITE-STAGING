...
class FootcareDetails extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      FootMaster: [],
  ...
  componentDidMount() {
    ...
    PostApiCall.postRequest(
      {
        id: this.props.match.params.footid,
      },
      "GetFootwearMasterDetailsWebsite"
    ).then((results) =>
      results.json().then((obj) => {
        if (results.status == 200 || results.status == 201) {
          ...
          PostApiCall.postRequest(
            ...
            "GetFootwearGenderListingWebsite"
          ).then((results1) =>
    ...
    PostApiCall.postRequest(
      ...
      "GetFootwearVariantDetailsWebsite"
    ).then((results) =>
      results.json().then((obj) => {
        if (results.status == 200 || results.status == 201) {
          ...
          var size = [];
          for (var i = 0; i < Object.keys(obj.data).length; i++) {
            size.push(parseInt(obj.data[i].fld_size));
      ...
      PostApiCall.postRequest(
        {
          ...
          url : `/footwear/${
            this.props.match.params.footid +
            "/" +
            this.props.match.params.varid +
            "/" +
            this.props.match.params.footname
                                ...
                                onClick={() => {
                                  for (
                                    ...
                                    if (
                                      ...
                                      window.location.href = `/footwear/${
                                        this.state.FootVariants[i].fld_footid +
                                        "/" +
                                        this.state.FootVariants[i].fld_id +
                                        "/" +
                                        this.state.FootVariants[i].fld_name.replace(/\W|_/g,"")
