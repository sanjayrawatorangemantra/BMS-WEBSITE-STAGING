...
class SocksListing extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      Foot: [],
    ...
    GetApiCall.getRequest("GetSocksListing").then((results) => {
      results
        ...
        GetApiCall.getRequest("GetSocksColorDataWebsite").then(resultdes =>
          ...
          GetApiCall.getRequest("GetSocksBrandData").then(resultdes =>
            resultdes.json().then(obj => {
                this.setState({
        ...
        GetApiCall.getRequest("GetSocksSizeDataWebsite").then(resultdes =>
          resultdes.json().then(obj => {
          ...
          GetApiCall.getRequest("GetFilterSocksListing").then((results) => {
            results
              .json()
              .then((data) => ({
                data: data,
   ...
   src={info.fld_mobileimage}/>   
        ...
        <div class="container doctors-section">
          ...
          <div class="col-md-3 col-sm-3 col-xs-3 col-3">
              ...
              <div class="brands">
                  <h5 style={{borderTop: '0px',paddingTop:'0px'}}>Socks <br/>( {this.state.FootDetails.length} Products )</h5>
                  </div>
                ...
                <div class="brands">
                  ...
                  <ul  id="style-3">
                    {this.state.SizeData.map((dt,i)=>(
                            <li>
                            <input
                            checked={this.state.SizeFilter.includes(dt.fld_size) ? true : false}
                              type="checkbox"
                              name="checkbox3"
                              id="checkbox3"
                              class="css-checkbox"
                              onChange={()=>{
                                ...
                                if(ar.includes(dt.fld_size)){
      
                                  ar.splice(ar.indexOf(dt.fld_size),1)
